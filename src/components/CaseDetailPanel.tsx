...
export function CaseDetailPanel({ caseId, onBack }: CaseDetailPanelProps) {
  const { user } = useAuth();
    ...
    <div className="flex-1 overflow-y-auto p-8 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-medium truncate">{caseData.client_name}</h1>
          ...
          {caseData.case_number && (
            <p className="text-sm text-muted-foreground font-mono mt-0.5">{caseData.case_number}</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {caseData.court && (
          <div className="bg-card border border-border rounded-sm p-4">
            ...
            {caseData.court_division && <p className="text-xs text-muted-foreground">{caseData.court_division}</p>}
          </div>
        )}
        {caseData.area_of_law && (
          <div className="bg-card border border-border rounded-sm p-4">
            ...
            <p className="text-sm font-medium">{caseData.area_of_law}</p>
          </div>
        )}
        {caseData.state && (
          <div className="bg-card border border-border rounded-sm p-4">
        ...
        <div className="bg-card border border-border rounded-sm p-4">
          ...
          <p className="text-xs text-muted-foreground">Atualizado: {formatDate(caseData.updated_at)}</p>
        </div>
      </div>
      ...
      {caseData.description && (
        <div className="bg-card border border-border rounded-sm p-5">
          <h3 className="text-sm font-medium mb-2">Descrição do Processo</h3>
      ...
      <div className="bg-card border border-border rounded-sm p-5">
        ...
        {docs.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-6">Nenhum documento vinculado a este processo.</p>
        ) : (
      ...
      <div className="bg-card border border-border rounded-sm p-5">
        ...
        {conversations.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-6">Nenhuma conversa vinculada a este processo.</p>
        ) : (
          <div className="space-y-2">
            {conversations.map((conv) => (
              <div key={conv.id} className="flex items-center gap-3 p-3 border border-border bg-muted/30 rounded-sm">
                <MessageSquare className="size-4 text-primary" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{conv.title || "Conversa sem título"}</p>
                  <p className="text-xs text-muted-foreground">{conv.message_count} mensagens · {formatDate(conv.updated_at)}</p>
                </div>
              </div>
            ))}
          </div>
